...
class RoadIntersectionSplineControlEnv(RoadIntersectionControlEnv):
    ...
    def get_starting_positions(self, as_tensor: bool = False):
        pos = {a_id: self.agents[a_id]["vehicle"].position.clone().unsqueeze(0)
    ...
    def register_dynamics_track(
        ...
        for a_id in self.get_agent_ids_list():
            self.agents[a_id]["dynamics"] = ParametricBicycleKinematicsModel(
                tracks[a_id], motion, model_kwargs={"p_num": 5, "alpha": 1.0}
            )
    ...
    def compute_loss(
        ...
        return (
            5.0 * dist / ((self.agents[a_id]["original_distance"] + 1e-12) * self.horizon)
            + 1.0 * theta_diff / self.horizon
            + 100.0 * collision / self.horizon  # Additional weightage on collision
        )
    ...
    def step(
        ...
        **kwargs,
        ...
        prev_states = {a_id: self.get_agent_driving_state(a_id)
                       for a_id in id_list}
            ...
            for a_id in id_list:
                if not self.is_agent_done(a_id):
                    ...
                    self.world.update_state(
                        a_id, state, change_road_association=(i == timesteps - 1)
                    )
                    ...
                    if differentiable_objective:
                        ...
                        losses[a_id] += self.compute_loss(
                            a_id, state, nactions[a_id][i, :], prev_states[a_id]
                        )
    ...
    def _get_distances(
        ...
        t = torch.max(torch.zeros_like(t), torch.min(torch.ones_like(t), t)).unsqueeze(1)
    ...
    def smooth_collision_penalty(self, a_id: str, pos = None):
        ...
        distance = self._get_distances(vehicle.position if pos is None else pos, pt1, pt2)
