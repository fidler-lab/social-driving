...
class MultiAgentHighwayBicycleKinematicsModel(
    ...
    def get_observation_space(self):
        return Tuple(
            [
                Box(
                    low=np.array(
                        [0.0, -1.0, 0.5, 0.5] * self.history_len
                    ),
                    high=np.array(
                        [1.0, 1.0, 1.0, 1.0] * self.history_len
                    ),
                ),
    ...
    def get_state(self):
        ...
        obs = torch.cat([
            inv_dist, speed / self.dynamics.v_lim, self.accln_rating, self.vel_rating
        ], -1)
    ...
    def discrete_to_continuous_actions(self, actions: torch.Tensor):
        actions[:, 1:] = actions[:, 1:] * self.max_accln * self.accln_rating.to(
            actions.device
        )
        return actions
